...
(function () {
    ...
    var _cssText = [
        // --- 月宫触发器基础 (尺寸缩小2倍) ---
        '#moon-trigger{position:fixed;top:20px;left:20px;z-index:2147483647;width:65px;height:75px;cursor:grab;user-select:none;touch-action:none;transition:transform .25s cubic-bezier(.34,1.56,.64,1)}',
        '#moon-trigger.dragging{cursor:grabbing}',
        // --- 水面倒影 ---
        '.water-surface{position:absolute;bottom:13px;left:50%;transform:translateX(-50%);width:55px;height:15px;z-index:2;pointer-events:none}',
        ...
        '.water-ripple.r1{width:30px;height:6px;animation-delay:0s}',
        '.water-ripple.r2{width:30px;height:6px;animation-delay:-1.3s}',
        '.water-ripple.r3{width:30px;height:6px;animation-delay:-2.6s}',
        '@keyframes rippleExpand{0%{width:20px;height:4px;opacity:.6}100%{width:50px;height:10px;opacity:0}}',
        '.water-reflection{position:absolute;bottom:3px;left:50%;transform:translateX(-50%) scaleY(-.35);width:35px;height:35px;opacity:.25;filter:blur(3px);mask-image:linear-gradient(to bottom,rgba(0,0,0,.8),transparent 70%);-webkit-mask-image:linear-gradient(to bottom,rgba(0,0,0,.8),transparent 70%)}',
        // --- 月宫 ---
        '.moon-paradise{position:absolute;top:5px;left:50%;transform:translateX(-50%);width:42px;height:42px;z-index:4;animation:moonFloat 5.5s ease-in-out infinite;will-change:transform}',
        '@keyframes moonFloat{0%,100%{transform:translateX(-50%) translateY(0)}50%{transform:translateX(-50%) translateY(-3px)}}',
        ...
        '@keyframes moonHover{0%,100%{transform:translateX(-50%) translateY(-2px) scale(1.05)}50%{transform:translateX(-50%) translateY(-5px) scale(1.08)}}',
        '.moon-glow-outer{position:absolute;inset:-13px;background:radial-gradient(ellipse at center,rgba(240,235,220,.35),transparent 65%);border-radius:50%;animation:moonGlowBreath 4s ease-in-out infinite}',
        '.moon-glow-inner{position:absolute;inset:-5px;background:radial-gradient(ellipse at center,rgba(250,245,232,.45),transparent 60%);border-radius:50%;animation:moonGlowBreath 3s ease-in-out infinite reverse}',
        ...
        // --- 云海环绕 ---
        '.cloud-sea{position:absolute;inset:-8px -5px;pointer-events:none;z-index:3}',
        ...
        '.cloud-layer.cl1{width:28px;height:11px;top:35%;left:-4px;animation:cloudDrift1 8s ease-in-out infinite}',
        '.cloud-layer.cl2{width:30px;height:12px;bottom:28%;right:-3px;animation:cloudDrift2 9s ease-in-out infinite}',
        '.cloud-layer.cl3{width:21px;height:8px;top:20%;right:2px;animation:cloudDrift1 6.5s ease-in-out infinite reverse;opacity:.75}',
        '.cloud-layer.cl4{width:24px;height:9px;bottom:38%;left:-2px;animation:cloudDrift2 8.5s ease-in-out infinite reverse;opacity:.65}',
        '.cloud-layer.cl5{width:19px;height:7px;top:55%;right:4px;animation:cloudDrift1 7.5s ease-in-out infinite;opacity:.55}',
        '@keyframes cloudDrift1{0%,100%{transform:translateX(0) translateY(0);opacity:.65}50%{transform:translateX(5px) translateY(-2px);opacity:.9}}',
        '@keyframes cloudDrift2{0%,100%{transform:translateX(0) translateY(0);opacity:.55}50%{transform:translateX(-4px) translateY(3px);opacity:.85}}',
        // --- 月相环 ---
        ...
        '.phase-ring.pr-outer{width:58px;height:58px;border:1px dashed rgba(80,70,50,.12);animation:phaseOrbit 30s linear infinite}',
        '.phase-ring.pr-inner{width:45px;height:45px;border:1px solid rgba(80,70,50,.08);animation:phaseOrbit 20s linear infinite reverse}',
        '@keyframes phaseOrbit{from{transform:translate(-50%,-50%) rotate(0deg)}to{transform:translate(-50%,-50%) rotate(360deg)}}',
        '.orbit-star{position:absolute;width:2px;height:2px;background:#fff;border-radius:50%;box-shadow:0 0 5px #fff,0 0 10px rgba(255,255,255,.5);animation:starTwinkle 2.5s ease-in-out infinite;will-change:opacity}',
        '.orbit-star.os1{top:2px;left:50%;transform:translateX(-50%)}',
        '.orbit-star.os2{top:50%;right:0;transform:translateY(-50%);animation-delay:-.7s}',
        '.orbit-star.os3{bottom:5px;left:50%;transform:translateX(-50%);animation-delay:-1.4s}',
        ...
        // --- 速度印章 ---
        '.moon-seal-token{position:absolute;bottom:4px;right:3px;min-width:22px;height:16px;padding:0 4px;background:#a0392f;color:#f8f2e6;font-size:6px;font-weight:700;border-radius:2px;display:flex;align-items:center;justify-content:center;font-family:"Noto Serif SC","SimSun",serif;box-shadow:0 4px 14px rgba(160,57,47,.3),inset 0 1px 0 rgba(255,255,255,.2);z-index:10;letter-spacing:.5px;transform:rotate(-5deg);animation:sealFloat 4s ease-in-out infinite;border:1px solid rgba(255,255,255,.15);will-change:transform}',
        '.moon-seal-token::before{content:"";position:absolute;inset:2px;border:1px solid rgba(248,242,230,.3);border-radius:1px}',
        '.moon-seal-token::after{content:"";position:absolute;top:2px;left:3px;right:3px;height:2px;background:linear-gradient(90deg,rgba(255,255,255,.3),transparent);border-radius:1px}',
        '@keyframes sealFloat{0%,100%{transform:rotate(-5deg) translateY(0)}50%{transform:rotate(-3deg) translateY(-2px)}}',
        // --- 水墨涟漪 ---
        ...
        // --- 面板卷轴 ---
        // ====================================================================
        '#moon-panel{position:fixed;top:85px;left:20px;z-index:2147483646;width:360px;visibility:hidden;pointer-events:none;perspective:1500px}',
        ...
        '@media (max-width:480px){' +
            // 触发器缩小
            '#moon-trigger{width:45px;height:52px;top:12px;left:10px}' +
            '.moon-paradise{width:29px;height:29px;top:3px}' +
            '.moon-glow-outer{inset:-7px}' +
            '.moon-glow-inner{inset:-3px}' +
            '.water-surface{width:38px;bottom:9px}' +
            '.water-ripple.r1,.water-ripple.r2,.water-ripple.r3{width:23px;height:5px}' +
            '.water-reflection{width:24px;height:24px}' +
            '.moon-seal-token{min-width:17px;height:12px;padding:0 3px;font-size:5px;bottom:2px;right:1px;letter-spacing:.3px}' +
            '.cloud-layer.cl1{width:19px;height:8px}' +
            '.cloud-layer.cl2{width:21px;height:8px}' +
            '.phase-ring.pr-outer{width:40px;height:40px}' +
            '.phase-ring.pr-inner{width:28px;height:28px}' +
            '.orbit-star{width:2px;height:2px}' +

            // 面板缩小 - 移动端固定宽度260px
            '#moon-panel{top:58px;left:10px;width:260px;max-width:260px}' +
        ...
        '@media (max-width:374px){' +
            '#moon-trigger{width:38px;height:44px;top:10px;left:8px}' +
            '.moon-paradise{width:24px;height:24px}' +
            '.moon-glow-outer{inset:-5px}' +
            '.moon-glow-inner{inset:-3px}' +
            '.water-surface{width:30px;bottom:7px}' +
            '.water-ripple.r1,.water-ripple.r2,.water-ripple.r3{width:18px;height:4px}' +
            '.water-reflection{width:20px;height:20px}' +
            '.moon-seal-token{min-width:14px;height:10px;padding:0 3px;font-size:5px;bottom:2px;right:1px}' +
            '.cloud-layer.cl1{width:15px;height:6px}' +
            '.cloud-layer.cl2{width:17px;height:7px}' +
            '.phase-ring.pr-outer{width:33px;height:33px}' +
            '.phase-ring.pr-inner{width:25px;height:25px}' +

            '#moon-panel{top:50px;left:8px;width:240px;max-width:240px}' +
    ...
    function _updateUI() {
        ...
        if (triggerMoon) triggerMoon.innerHTML = _getPhaseInnerSVG(phaseIdx, 42);
        ...
        // 更新倒影
        if (_moonReflection) {
            _moonReflection.innerHTML = '<svg viewBox="0 0 42 42" style="width:100%;height:100%">' + _getPhaseInnerSVG(phaseIdx, 42) + '</svg>';
        }
    ...
    function _mountUI() {
        ...
        moonParadise.innerHTML =
            ...
            '<svg viewBox="0 0 42 42" id="mbTriggerMoon" style="width:100%;height:100%;position:relative;z-index:2">' +
            '<defs>' +
            ...
            '<circle cx="21" cy="21" r="16.5" fill="url(#tpMoonGrad)"/>' +
            '<g opacity=".1" fill="#7a7060">' +
            '<ellipse cx="16" cy="17" rx="2.7" ry="2.2"/>' +
            '<ellipse cx="25" cy="23" rx="3.5" ry="2.7"/>' +
            '<ellipse cx="19" cy="28" rx="2.3" ry="1.6"/>' +
            '<circle cx="27" cy="15" r="1.1"/>' +
            '</g>' +
            '<circle cx="21" cy="21" r="16.5" fill="none" stroke="rgba(100,90,70,.18)" stroke-width=".5" opacity=".4"/>' +
            '</g>' +
            '</svg>';
            ...
            function onPointerMove(e) {
                ...
                if (!rafPending) {
                    ...
                    requestAnimationFrame(function () {
                        ...
                        var tw = _trigger.offsetWidth || 65;
                        var th = _trigger.offsetHeight || 75;
