...
(function() {
    ...
    function injectCache() {
        ...
        // === 补丁1：包装 addCallback，为所有回调添加 null 保护 ===
        // 防止 cc.js 内部回调在 finished(null) 时崩溃
        var originalAddCallback = LoadManager.prototype.addCallback;
        if (originalAddCallback) {
            LoadManager.prototype.addCallback = function(caller, callback, args) {
        ...
        if (originalFinish) {
            LoadManager.prototype.finished = function(t) {
                // 图片加载失败时（t 为 null），直接调用原始方法，不尝试缓存
                if (!t) {
                    try {
                        return originalFinish.call(this, t);
                    } catch (err) {
